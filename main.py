...
from my_podcast import fetch
from podcast_player import open_vlc
...
import pandas as pd
from podcast import retrive_definition
import winsound

USERNAME = config('USER')
BOTNAME = config('BOTNAME')
HOUNDIFY_CLIENT_ID = config('HOUNDIFY_CLIENT_ID')
HOUNDIFY_CLIENT_KEY = config('HOUNDIFY_CLIENT_KEY')
LOCAL_TARGET_DIRECTORY = config('LOCAL_TARGET_DIRECTORY')
...
def podcast():
    speak('Which podcast would you like to get?')
    query = take_user_input().lower()
    print(query)
    speak(f"Getting the latest podcast for {query}")
    retval = retrive_definition(query)
    file = fetch(retval, LOCAL_TARGET_DIRECTORY)
    open_vlc(LOCAL_TARGET_DIRECTORY + "//" + file)
